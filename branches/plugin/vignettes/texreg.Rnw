...
Moreover, most existing packages are not able to merge multiple models into one single table.
The \TeX reg package fills these gaps.
Currently, \texttt{lm}, \texttt{lme}, \texttt{gls}, \texttt{glm} and \texttt{ergm} objects are supported.
New model types can be easily implemented.
...
@
If this is not possible for some reason, the source files can be downloaded from \url{http://www.philipleifeld.de} or \url{http://r-forge.r-project.org/projects/texreg/}.
...
An example:
<<results=tex>>=
table <- texreg(list(m1, m2), use.packages=FALSE, label="tab:3", caption="My regression table", scriptsize=TRUE)
@
...
\section{Creating templates for new model types}
