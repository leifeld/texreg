...
\begin{longtable}{l l l l p{5.4cm}}
\toprule
Class		& From package				& Contributed by	& Added		& Description \\
\midrule
\endhead
\texttt{ergm}	& \texttt{statnet}, \texttt{ergm}	& Philip Leifeld	& 2012-06-18	& Exponential random graph models \\
\texttt{lm}	& \texttt{base}				& Philip Leifeld	& 2012-06-19	& Ordinary least squares (OLS, linear models) \\
\texttt{glm}	& \texttt{base}				& Philip Leifeld	& 2012-06-19	& Generalized linear models \\
\texttt{gls}	& \texttt{nlme}				& Philip Leifeld	& 2012-06-19	& Generalized least squares \\
\texttt{lme}	& \texttt{nlme}				& Philip Leifeld	& 2012-06-19	& Linear mixed effects models (random effects, fixed effects) \\
...
extract.lm <- function(model) {
  ...
  }                                     #do not forget to replace "lm object"
...
\end{verbatim}
After writing a custom function, the function has to be registered in the \texttt{texreg()} function.
Download a copy of the texreg.R file containing the texreg() function from the R-Forge website (see section~\ref{source}).
Modify the \texttt{texreg()} function according to the following guidelines, then execute it once.
...
First, you have to add something like this in order to let \TeX reg properly check whether the model type is known:
\begin{verbatim}
      class(l)[1] == "gls" | 
\end{verbatim}
Of course, ``gls'' must be replaced by the model type of your choice. Then, something like this must be added some lines below the first modification:
\begin{verbatim}
    } else if (class(l[[i]])[1] == "gls") {
      model <- extract.gls(l[[i]])
      models <- append(models, list(model))
\end{verbatim}
Again, replace ``gls'' twice with the model type you want to implement.

If you write a new extract function, it would be very helpful to post them in the forum (see section \ref{help}) in order to let other users profit from it.
