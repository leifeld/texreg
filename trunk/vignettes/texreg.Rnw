...
Currently, \texttt{lm}, \texttt{lme}, \texttt{gls}, \texttt{glm}, \texttt{ergm}, \texttt{plm}, \texttt{pmg}, and \texttt{rq} objects are supported.
...
Currently, \TeX reg supports \texttt{lm}, \texttt{lme}, \texttt{gls}, \texttt{glm}, \texttt{ergm}, \texttt{plm}, \texttt{pmg}, and \texttt{rq} objects.
...
Download a copy of the texreg.R file containing the texreg() function from the R-Forge website (see section~\ref{installation}).
Modify the texreg() function according to the following guidelines, then execute it once.
