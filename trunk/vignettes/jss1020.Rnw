...
In spite of this, common approaches for linking \proglang{R} model output and tables in \proglang{\LaTeX} include copying and pasting individual values after every change of the model, custom user-written functions which convert a specific model into a matrix, the use of sophisticated table-management packages (see next section), and the inclusion of single models in the form of the model summary instead of nicely aligned coefficient tables as a second best solution.
...
This article introduces the \pkg{texreg} package (version 1.29.4), which closes this gap and provides a unified framework for typesetting \proglang{\LaTeX} and \proglang{HTML} tables for various statistical models in \proglang{R}.
...
In yet other communities, journals or models, $p$~values or significance stars are not required or even deemed inappropriate \citep[see the \pkg{lme4} package by][]{bates2013lme4:}.
...
In other situations, it may be important to omit certain coefficients (like random or fixed effects or thresholds), reorder the coefficients in the model (e.g., because some models put interaction effects at the end of the list of coefficients), or replace coefficients, standard errors, or $p$~values by custom vectors, for example when heteroskedasticity-consistent (``robust'') standard errors are used \citep{zeileis2004econometric}.
...
The \pkg{memisc} package features \code{aftreg}, \code{betareg}, \code{clm}, \code{dynml}, \code{glm}, \code{hurdle}, \code{ivreg}, \code{lm}, \code{lmer}, \code{mer}, \code{multinom}, \code{phreg}, \code{polr}, \code{tobit}, \code{simex}, \code{survreg}, \code{weibreg}, and \code{zeroinfl} models but cannot handle any network models and recent versions of \pkg{lme4} multilevel models \citep{bates2013lme4:} (as of May 22, 2013).
...
\code{dynlm}		& \pkg{dynlm}			& Time series regression with ``ts'' data \\
...
\code{[gl|l|nl]merMod}	& \pkg{lme4} (old)		& Generalized linear mixed models \\
...
\code{lme4}		& \pkg{lme4} (new)		& Linear mixed-effects models \\
...
\end{tabular}
\end{center}
\caption{List of 51 currently supported model types (as of September 13, 2013).}
\label{tab:types}
\end{table}
...
For example, the \code{extract.lme4()} function for \pkg{lme4} multilevel models \citep{bates2013lme4:} has Boolean options like \code{include.variance}, which turns on the inclusion of random effect variances in the goodness-of-fit block, and numeric arguments like \code{conf.level}, which sets the confidence level for computation of profile or bootstrapped confidence intervals.
...
As some types of statistical models report confidence intervals rather than standard errors and $p$ values, the \code{texreg} class definition can alternatively store lower and upper bounds of confidence intervals instead of standard errors and $p$ values.
Which slots of the class are used depends on the \code{extract} function for the specific model.
The functions in the \pkg{texreg} package check whether standard errors are present in the \code{texreg} object and use either standard errors or confidence intervals depending on availability.

The \code{texreg} class contains validation rules which make sure that the four coefficient vectors all have the same length and that the three GOF vectors also all have the same length.
There are several exceptions to this rule: the standard errors and $p$~values, the confidence intervals, and the decimal-place vector are optional and may also have a length of zero.
However, either standard errors or confidence intervals must be present.
...
Most model types implemented in \pkg{texreg} report standard errors and $p$ values.
However, some model types report confidence intervals by default.
The \pkg{btergm} package \citep{leifeld2013btergm:} and the \pkg{lme4} package \citep{bates2013lme4:} are two examples.
If confidence intervals are preferred to standard errors but not available by default, the \code{ci.force} argument allows conversion from standard errors to confidence intervals.

A star is added to estimates where the confidence interval does not contain the value given by \code{ci.test} (to remove significance stars, \code{ci.test = NULL} can be set).
When the \code{bold} argument is used in conjunction with confidence intervals, \code{bold} values greater than $0$ cause \pkg{texreg} to print estimates in bold where the \code{ci.test} value is outside the confidence interval, regardless of the actual value of the \code{bold} argument.
The \code{ci.force.level} argument determines at which confidence level the interval should be computed:
<<results=tex>>=
texreg(
    list(m1, m1, m2), 
...
\begin{figure}[ht!]
  \begin{center}
    \includegraphics[width=0.7\textwidth]{jss1020-figure}
  \end{center}
...
Note also that either the \code{se} slot or the \code{ci.low} and \code{ci.up} slots must contain values.
...
\begin{table}[t]
 \begin{center}
  \begin{tabular}{l p{12.3cm}}
   ...
   \code{ci.low}	& (\emph{optional}) Lower bounds of the confidence intervals. An alternative to the \code{se} slot if confidence intervals are preferred. \\
   \code{ci.up}		& (\emph{optional}) Upper bounds of the confidence intervals. An alternative to the \code{se} slot if confidence intervals are preferred. \\
...
For example, the user can decide whether random effect variances should be included in \pkg{texreg} tables of \code{lme4} objects \citep[from the \pkg{lme4} package, see][]{bates2013lme4:} by setting the \code{include.variance} argument.
