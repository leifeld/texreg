...
#'
#' Aggregate models and GOFs from `texreg` objects into a common matrix.
#'
#' This function takes a list of `texreg` objects, as produced by the
#' [extract()] function, and produces a matrix, either for the
#' coefficient block (`returnobject = "m"`) (in numeric form) or for the
#' goodness-of-fit (GOF) block of the table that is to be created
#' (`returnobject = "gof"`; in formatted character form). On the way, also
...
#'
#' @param models A list of `texreg` objects, as created by the
#'   \link{get.data} function.
#' @param gof.names A character vector of names for the GOF statistics, as
#'   created by the [get.gof()] function. This is needed to attach row
#'   names to the GOF block or decimal matrix.
#' @param custom.gof.names A character vector of replacement names for the GOF
#'   statistics. Must have the same length as `gof.names`. Can be
#'   `NULL` if the original GOF names should be kept.
...
#'   the GOF entries, for example `c(1, 2, 4, 3)` for swapping the last two
#'   entries.
...
#'   \describe{
#'     \item{`"m"`}{for creating the coefficient block}
#'     \item{`"gof.matrix"`}{for creating the GOF block}
#'   }
#'
...
#' @seealso [extract()], [get.data()],
#'   [get.gof()], [coeftostring()],
#'   [replaceSymbols()]
...
#' generated by the [outputmatrix()] function.
#'
#' @param x A character vector with coefficients (e.g., `"5.03 ***"`) and
#'   either standard errors (e.g., `"(0.22)"`) or confidence intervals
#'   (e.g., `"[-0.98; 0.24]"`).
#' @param single.row Are both the coefficient and the uncertainty measure in the
#'   same row, as per the `single.row` argument of the table creation
#'   functions?
...
#' @keywords internal
#' @author Philip Leifeld
#' @seealso [outputmatrix()], [matrixreg()]
