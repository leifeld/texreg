...
\usepackage{booktabs,dcolumn,rotating,paralist,thumbpdf,longtable}
...
\caption[List of 55 supported model types (version 1.32).]{\label{tab:types} List of 55 supported model types (version 1.32).} \\
\endlastfoot
...
\pkg{texreg} (version 1.32), in contrast, can deal with all of the
...
`\code{btergm}',`\code{coxph.penal}', `\code{fGARCH}', `\code{gamlss}', 
`\code{gmm}', `\code{lme}', `\code{lme4}', `\code{lnam}', `\code{maBina}', 
`\code{nlme}', `\code{rem.dyad}', `\code{rq}', `\code{sclm}', 
`\code{stergm}', `\code{systemfit}', `\code{texreg}' and `\code{zelig}' 
(`\code{logit}' and `\code{relogit}') objects.  Table~\ref{tab:types}
gives an overview of currently implemented model types.
...
by default.  The \pkg{btergm} package \citep{leifeld2014btergm:} and
...
G{\'a}bor Uhrin, Antoine Vernet, Yanghao Wang, and Yihui Xie for their
