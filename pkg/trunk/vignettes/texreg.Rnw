...
@
If this is not possible for some reason, the source files can be downloaded from \url{http://www.philipleifeld.de} or \url{texreg.r-forge.r-project.org}.
...
Questions can be posted to a public forum at \url{texreg.r-forge.r-project.org}.
