...
\title{texreg: Conversion of Statistical Model Output in R to \LaTeX\ and HTML Tables\thanks{The author would like to thank S.\,Q. Chang, Skyler Cranmer, Sebastian Daza, Lena Koerber, Johannes Kutsam, Fabrice Le Lec, Francesco Sarracino, Matthieu Stigler, Sebastian Ugbaje, G{\'a}bor Uhrin, Yanghao Wang, and Yihui Xie for valuable input.}}
...
\section{Motivation}
The \texttt{texreg} package for the statistical computing environment \texttt{R} was designed to convert regression model output from multiple models into tables for inclusion in \LaTeX\ or HTML documents.
It is an alternative to packages like \texttt{xtable}, \texttt{apsrtable}, \texttt{outreg}, \texttt{stargazer} and \texttt{memisc}, which can also convert \texttt{R} tables to \LaTeX\ tables.
Only a subset of these packages is able to merge multiple regression models in a single table.
Those packages which can do this do not support important model types such as \texttt{lme} or \texttt{mer} (linear mixed effects models) and \texttt{ergm} objects (exponential random graph models from the \href{http://www.statnet.org}{statnet} suite of packages), or are not customizable and extendable.
\texttt{texreg}, in contrast, accepts these model types and can also merge multiple models in a single table.
...
\texttt{texreg} can be used within Sweave and knitr.
...
\midrule
\texttt{aftreg}		& \texttt{eha}			& 2013-03-23	& Accelerated failure time regression \\
\texttt{betareg}	& \texttt{betareg}		& 2013-03-13	& Beta regression for rates and proportions \\
...
\texttt{gam}		& \texttt{mgcv}			& 2013-03-13	& Generalized additive models \\
...
\texttt{ivreg}		& \texttt{AER}			& 2013-03-13	& Instrumental-variable regression using 2SLS \\
\texttt{hurdle}		& \texttt{pscl}			& 2013-03-13	& Hurdle regression models for count data \\
...
\texttt{multinom}	& \texttt{nnet}			& 2013-03-13	& Multinomial log-linear models \\
...
\texttt{phreg}		& \texttt{eha}			& 2013-03-23	& Parametric proportional hazards regression \\
...
\texttt{rem.dyad}	& \texttt{relevent}		& 2013-02-28	& Relational event model for dyadic data \\
...
\texttt{survreg}	& \texttt{survival}		& 2013-03-13	& Parametric survival regression models \\
\texttt{survreg.penal}	& \texttt{survival}		& 2013-03-13	& Frailty survival models \\
...
\texttt{weibreg}	& \texttt{eha}			& 2013-03-23	& Weibull regression \\
\texttt{zeroinfl}	& \texttt{pscl}			& 2013-03-13	& Zero-inflated regression models \\
\bottomrule
\end{tabular}
\end{center}
...
Beside \LaTeX\ output, \texttt{texreg} can also export nicely formatted tables to MS Word files, HTML files (which can be viewed in any web browser), or it can print nicely formatted regression tables directly to the screen (that is, to the \texttt{R} console) for easier model comparison.

\section{Installation}\label{installation}
It should be possible to install \texttt{texreg} using a simple command:
...
To load the package in \texttt{R} once it has been installed, enter the following command:
<<results=hide>>=
library(texreg)
@
...
help(texreg)
@
If all else fails, more help can be obtained from the homepage of the \texttt{texreg} package.
...
To improve the situation, the following commands can do this automatically (the \LaTeX\ output code is shown below the \texttt{R} code, and the resulting table is shown in table \ref{tab:1}):
<<>>=
library(texreg)
texreg(m2, booktabs=TRUE, dcolumn=TRUE)
@
<<echo=FALSE,results=tex>>=
texreg(m2, use.packages=FALSE, label="tab:1", booktabs=TRUE, dcolumn=TRUE, float.pos="p")
@
Note that the \texttt{booktabs} and \texttt{dcolumn} arguments were used to create more fancy tables than the default behavior using the \texttt{booktabs} and the \texttt{dcolumn} \LaTeX\ packages (for nicer top and bottom rules, and for decimal point alignment of coefficients and standard errors).
The resulting table is printed directly to the \texttt{R} console for easy copy \& paste.
...
Note the additional \texttt{caption} argument to set the caption of the table.
<<eval=FALSE>>=
texreg(list(m1,m2), booktabs=TRUE, dcolumn=TRUE, caption="Multiple models")
@
<<results=tex,echo=FALSE>>=
texreg(list(m1,m2), booktabs=TRUE, dcolumn=TRUE, use.packages=FALSE, label="tab:2", float.pos="p", caption="Multiple models")
@
Tables~\ref{tab:1} and~\ref{tab:2} consume more horizontal space than is actually necessary.
This is due to the long note regarding significance thresholds for the $p$ values.
To avoid this problem, the note can be replaced by a shorter note using the \texttt{custom.note} argument.

The \texttt{texreg} package contains many customizations.
...
An example:
<<results=tex>>=
texreg(list(m1, m2), booktabs=TRUE, dcolumn=TRUE, use.packages=FALSE, 
    label="tab:3", caption="Custom coefficient and model names", 
    custom.coef.names=c("(Intercept)",  "Treatment", "Control"), 
    custom.model.names=c("First model", "Second model"), float.pos="p")
@
...
The \texttt{texreg} package can also handle \texttt{ergm} objects (that is, exponential random graph models, which are used in social network analysis).
Here is an example:
...
@
The \texttt{texreg} command can then be used to create a table with the coefficients.
The \texttt{summary} function for \texttt{ergm} objects attaches centered dots to weakly significant model terms by default (that is, a $\cdot$ is used to denote $p$ values between $0.05$ and $0.1$).
This behavior can be imitated by modifying the \texttt{stars} argument and adding a fourth significance level as follows:
<<results=tex>>=
texreg(list(m4, m5, m6), use.packages=FALSE, label="tab:4", booktabs=TRUE, 
    dcolumn=TRUE, float.pos="p", caption="Centered dots for weak significance", 
    stars=c(0.001, 0.01, 0.05, 0.1))
@
Table \ref{tab:4} shows the result of this command.
Note that the legend under the table is now extended, but the model terms do not feature any $p$ values between $0.05$ and $0.1$ in this case.
...
<<results=tex>>=
texreg(list(m4, m5, m6), use.packages=FALSE, label="tab:5", booktabs=TRUE, 
    dcolumn=TRUE, single.row=TRUE, float.pos="p", caption="In a single row")
@
The result is shown in table \ref{tab:5}.
Note the difference between tables \ref{tab:4} and \ref{tab:5}.

The \texttt{texreg} command can also combine the output of different model types in a single table.
...
@
<<results=tex>>=
texreg(list(m3, m2, m6), label="tab:6", use.packages=FALSE, booktabs=TRUE, 
    dcolumn=TRUE, float.pos="p", caption="Mixing different kinds of models")
@
...
These can be switched on or off (see \verb+?extract+ for details).

Many people use robust standard errors.
To include include them in a \texttt{texreg} table, the original standard errors can be replaced and new, custom values can be handed over.
...
So it is possible to use the \texttt{texreg} package not only with \LaTeX, but also with MS Office.
If the table is exported to a file, it is advisable to include the full header information of the HTML file.
An example:
<<results=hide>>=
htmlreg(list(m3, m2, m6), file="mytable.doc", inline.css=FALSE, doctype=TRUE, 
...
The default arguments should work well with \texttt{knitr} and HTML.
In addition to HTML, \texttt{knitr} is also compatible with Markdown, a simplified markup language.
\texttt{texreg} can work with Markdown as well, but an additional argument should be provided to make it work:
the \verb+star.symbol="\\*"+ argument makes sure that Markdown does not interpret the significance stars as special Markdown syntax.
The additional (and optional) \verb+center=TRUE+ argument centers the table horizontally on the page.
Here is an example:
<<results=hide>>=
htmlreg(list(m3, m2, m6), star.symbol="\\*", center=TRUE)
@

Finally, there is another function, which can print tables to the \texttt{R} console.
The command will nicely arrange the spaces etc. of your tables and will greatly facilitate model comparison (as a substitute of the default \texttt{summary} function).
To show the three models side-by-side in your \texttt{R} console, type the following code:
<<>>=
screenreg(list(m3, m2, m6))
@
This is especially useful to prepare a table on screen and eventually export it to \LaTeX\ or HTML.

\section{Creating templates for new model types}\label{templates}
Implementing new kinds of statistical models is fairly easy (if you know how to modify \texttt{R} functions).
...
\end{verbatim}
After executing the definition of the function and the adjusted \texttt{setMethod} command, \texttt{texreg} can be used with your models.
...
If it works and if you can provide a self-contained example, the code can be implemented in a future version of \texttt{texreg}.
...
Please send an inquiry if you are interested in joining the \texttt{texreg} project and working directly on the code.

\section{How to obtain the source code}\label{source}
If you would like to inspect the \texttt{texreg} source code in order to develop your own extensions, you can download the \texttt{.tar.gz} file from the repository homepage.
To do this, you can either search the list of R-Forge contributions (\url{http://download.r-forge.r-project.org/src/contrib/}) for \texttt{texreg}, or click on the ``R packages'' link on the \texttt{texreg} package homepage at R-Forge (\url{http://r-forge.r-project.org/projects/texreg/}).
Make sure you download the \texttt{texreg} file with the \texttt{.tar.gz} extension, open this compressed file (e.g., using 7Zip if you are on Windows), and open the \texttt{texreg.R} file in the \texttt{R/} directory.
