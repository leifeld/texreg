...
\title{texreg: Conversion of R regression output to \LaTeX\ tables\thanks{The author would like to thank S.Q. Chang, Skyler Cranmer, Sebastian Daza, Lena Koerber, Johannes Kutsam, Fabrice Le Lec, Francesco Sarracino, Matthieu Stigler, G{\'a}bor Uhrin, and Yanghao Wang for valuable input.}}
...
However, some additional arguments should be specified when \texttt{knitr} is used.
These arguments make sure that only the table is returned, without any header information:
<<results=hide>
htmlreg(list(m3, m2, m6), doctype=FALSE, html.tag=FALSE, head.tag=FALSE, 
    body.tag=FALSE, inline.css=TRUE)
@
In addition to HTML, \texttt{knitr} is also compatible with Markdown, a simplified markup language.
texreg can work with Markdown as well, but an additional argument should be provided to make it work:
...
The additional (and optional) \verb+align.center=TRUE+ argument centers the table horizontally on the page.
Here is an example:
<<results=hide>>=
htmlreg(list(m3, m2, m6), doctype=FALSE, html.tag=FALSE, head.tag=FALSE, 
    body.tag=FALSE, inline.css=TRUE, star.symbol="\\*", align.center=TRUE)
@
...
To show the three models side-by-side in your R console, type the following code:
<<>>=
