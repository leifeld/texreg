...
Currently supported model types are listed in section \ref{types}.
...
\section{Supported model types}\label{types}
\begin{longtable}{l l l l p{4.6cm}}
\toprule
Class			& Package			& Contributed by	& Added		& Description \\
\midrule
\endhead
\texttt{clm}		& \texttt{ordinal}		& Philip Leifeld	& 2012-10-12	& Cumulative link models \\
\texttt{clogit}		& \texttt{survival}		& Sebastian Daza	& 2012-09-30	& Conditional logistic regression \\
\texttt{coxph}		& \texttt{survival}		& Philip Leifeld	& 2012-10-14	& Cox proportional hazard models \\
\texttt{ergm}		& \texttt{ergm}			& Philip Leifeld	& 2012-06-18	& Exponential random graph models \\
\texttt{gee}		& \texttt{gee}			& Philip Leifeld	& 2012-10-14	& Generalized estimation equation \\
\texttt{glm}		& \texttt{stats}		& Philip Leifeld	& 2012-06-19	& Generalized linear models \\
\texttt{glmerMod}	& \texttt{lme4} (new)		& Philip Leifeld	& 2012-10-09	& Generalized linear mixed models \\
\texttt{gls}		& \texttt{nlme}			& Philip Leifeld	& 2012-06-19	& Generalized least squares \\
\texttt{lm}		& \texttt{stats}		& Philip Leifeld	& 2012-06-19	& Ordinary least squares \\
\texttt{lme}		& \texttt{nlme}			& Philip Leifeld	& 2012-06-19	& Linear mixed-effects models \\
\texttt{lmerMod}	& \texttt{lme4} (new)		& Philip Leifeld	& 2012-10-08	& Linear mixed-effects models \\
\texttt{lnam}		& \texttt{sna}			& Philip Leifeld	& 2012-10-07	& Linear network autocorrelation models \\
\texttt{mer}		& \texttt{lme4} (old)		& Philip Leifeld	& 2012-10-08	& Linear mixed-effects models \\
\texttt{nlmerMod}	& \texttt{lme4} (new)		& Philip Leifeld	& 2012-10-09	& Nonlinear mixed-effects models \\
\texttt{lrm}		& \texttt{rms}, \texttt{Design}	& Fabrice Le Lec	& 2012-07-04	& Logistic regression models \\
\texttt{plm}		& \texttt{plm}			& Lena Koerber		& 2012-08-01	& Linear models for panel data \\
\texttt{pmg}		& \texttt{plm}			& Lena Koerber		& 2012-08-01	& Linear panel models with heterogeneous coefficients \\
\texttt{polr}		& \texttt{MASS}			& Philip Leifeld	& 2012-10-12	& Ordered logistic or probit regression \\
\texttt{Relogit}	& \texttt{Zelig}		& Philip Leifeld	& 2012-10-14	& Rare events logistic regression \\
\texttt{rq}		& \texttt{quantreg}		& Lena Koerber		& 2012-08-01	& Quantile regression models \\
\texttt{sclm}		& \texttt{ordinal}		& Philip Leifeld	& 2012-10-12	& Cumulative link models \\
\texttt{svyglm}		& \texttt{survey}		& Philip Leifeld	& 2012-10-14	& Survey-weighted generalized linear models \\
\texttt{systemfit}	& \texttt{systemfit}		& Johannes Kutsam	& 2012-10-03	& Linear structural equations \\
\bottomrule \\
