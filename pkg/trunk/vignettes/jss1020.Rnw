...
\code{glmerMod}		& \pkg{lme4}			& Generalized linear mixed models \\
...
\code{lmerMod}		& \pkg{lme4}			& Linear mixed-effects models \\
...
\code{mer}		& \pkg{lme4}			& Linear mixed-effects models \\
...
\code{nlmerMod}		& \pkg{lme4}			& Nonlinear mixed-effects models \\
...
\end{tabular}
\end{center}
\caption{List of 49 currently supported model types (as of August 27, 2013).}
\label{tab:types}
\end{table}
...
For example, the \code{extract.mer()} function for \pkg{lme4} multilevel models \citep{bates2012lme4} has Boolean options like \code{include.variance} or \code{mcmc}, which turn on the inclusion of random effect variances in the goodness-of-fit block, and computation of Markov-chain-Monte-Carlo-based (MCMC) confidence intervals instead of the default na\"{\i}ve $p$ values, respectively.
...
The \pkg{btergm} package \citep{leifeld2013btergm:} is an example.
...
If the \code{ci.star} argument is set, a star is added to estimates where the confidence interval does not contain $0$.
Similarly, when the \code{bold} argument is used in conjunction with confidence intervals, \code{bold} values greater than $0$ cause \pkg{texreg} to bold estimates where $0$ is outside the confidence interval, regardless of the actual value of the \code{bold} argument.
The \code{ci.level} argument determines at which confidence level the interval should be computed:
<<results=tex>>=
texreg(
    list(m1, m1, m2), 
