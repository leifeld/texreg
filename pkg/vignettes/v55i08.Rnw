...
\Address{
  ...
  Eawag\\
  {\"U}berlandstr. 133\\
  8600 D{\"u}bendorf, Switzerland\\
  E-mail: \email{philip.leifeld@eawag.ch}\\
...
\caption[List of 71 supported model types (version 1.36).]{\label{tab:types} List of 71 supported model types (version 1.36).} \\
\endlastfoot
...
`\code{brglm}', `\code{btergm}', `\code{censReg}', `\code{coxph.penal}', 
`\code{ergmm}', `\code{ets}', `\code{fGARCH}', `\code{forecast}', 
`\code{gamlss}', `\code{glmmadmb}', `\code{glmrob}', `\code{felm}', 
`\code{gmm}', `\code{lme}', `\code{lme4}', `\code{lnam}', `\code{lqmm}', 
`\code{maBina}', `\code{mlogit}', `\code{mnlogit}', `\code{mtergm}', 
`\code{model.selection}', `\code{netlogit}', `\code{nlme}', 
`\code{rem.dyad}', `\code{rq}', `\code{sclm}', `\code{selection}', 
`\code{stergm}', `\code{systemfit}', and `\code{texreg}' objects. 
Table~\ref{tab:types} gives an overview of currently implemented model 
types.
