...
\caption[List of 64 supported model types (version 1.35).]{\label{tab:types} List of 64 supported model types (version 1.35).} \\
\endlastfoot
...
\pkg{texreg} (version 1.35), in contrast, can deal with all of the
...
model classes: `\code{averaging}', `\code{brglm}', `\code{btergm}', 
`\code{censReg}', `\code{coxph.penal}', `\code{ergmm}', `\code{fGARCH}', 
`\code{gamlss}', `\code{glmmadmb}', `\code{glmrob}', `\code{gmm}', 
`\code{lme}', `\code{lme4}', `\code{lnam}', `\code{maBina}', 
