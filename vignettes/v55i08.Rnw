...
\caption[List of 66 supported model types (version 1.36).]{\label{tab:types} List of 66 supported model types (version 1.36).} \\
\endlastfoot
...
\pkg{texreg} (version 1.36), in contrast, can deal with all of the
...
model classes: `\code{averaging}', `\code{bam}', `\code{brglm}', 
`\code{btergm}', `\code{censReg}', `\code{coxph.penal}', `\code{ergmm}', 
`\code{fGARCH}', `\code{gamlss}', `\code{glmmadmb}', `\code{glmrob}', 
`\code{gmm}', `\code{lme}', `\code{lme4}', `\code{lnam}', `\code{lqmm}', 
`\code{maBina}', `\code{mlogit}', `\code{mnlogit}', `\code{model.selection}', 
`\code{netlogit}', `\code{nlme}', `\code{rem.dyad}', `\code{rq}', 
`\code{sclm}', `\code{selection}', `\code{stergm}', `\code{systemfit}', 
and `\code{texreg}' objects. Table~\ref{tab:types} gives an overview of 
...
Michael Chirico, Skyler Cranmer, Sebastian Daza, Christopher Gandrud, 
Lena Koerber, Johannes Kutsam, Fabrice Le Lec, Florian Oswald, Markus Riester,
