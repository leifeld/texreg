...
`\code{censReg}', `\code{coxph.penal}', `\code{ergmm}', `\code{ets}', 
...
`\code{selection}', `\code{stergm}', `\code{systemfit}', and `\code{texreg}' 
objects. Table~\ref{tab:types} gives an overview of currently implemented model 
types.
