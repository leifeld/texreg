...
\caption[List of 97 supported model types (version 1.37).]{\label{tab:types} List of 97 supported model types (version 1.37).} \\
\endlastfoot
...
`\code{feglm}'			& \pkg{alpaca}			& GLM’s with high-dimensional k-way fixed effects \\
