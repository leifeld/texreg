...
\caption[List of 103 supported model types (version 1.36.24).]{\label{tab:types} List of 103 supported model types (version 1.36.24).} \\ \\
\endlastfoot
...
`\code{bife}'           & \pkg{bife}            & Binary Choice Models with Fixed Effects \\
...
`\code{feglm}'          & \pkg{alpaca}          & GLM with high-dimensional k-way fixed effects \\
...
`\code{gnls}'           & \pkg{nlme}            & Generalized non-linear least squares  \\
`\code{gmm}'            & \pkg{gmm}             & Generalized method of moments estimation \\
`\code{gnm}'            & \pkg{gnm}             & Generalized non-linear models \\
...
`\code{oglmx}'          & \pkg{oglmx}           & Ordered Generalized Linear Model \\
`\code{ols}'            & \pkg{rms}             & Ordinary least squares linear models \\
`\code{panelAR}'        & \pkg{panelAR}         & Linear models on panel data structures in the presence of AR(1)-type autocorrelation \\
...
\begin{figure}[ht!]
  ...
  \end{center}
  \caption{Result of the \code{plotreg} function. Significant estimates are colored in red and are represented by a circle point.}
  \label{fig:plotreg}
\end{figure}
