...
\caption[List of 79 supported model types (version 1.37).]{\label{tab:types} List of 79 supported model types (version 1.37).} \\
\endlastfoot
...
`\code{lmRob}'  		& \pkg{robust}		& Robust linear regression Models \\
...
`\code{summary.dynlm}'	& \pkg{dynlm}		& Summary objects for time series regression models \\
`\code{summary.ivreg}'	& \pkg{AER}		& Summary objects for instrumental-variable regression using 2SLS \\
`\code{summary.lm}'	    & \pkg{stats}		& Summary objects for Ordinary least squares  \\
